...
export default function DashboardStatCards() {
  ...
  const projections = [
    { label: '7 dias', value: stats.projected7, delta: stats.delta7 },
    { label: '14 dias', value: stats.projected14, delta: stats.delta14 },
    { label: '30 dias', value: stats.projected30, delta: stats.delta30 },
  ];

  return (
    <div className="space-y-3">
      {/* Row 1: Saldo + Fluxo semanal */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        {projections.map((p, i) => (
          <motion.div
            ...
            className={cn('card-elevated p-4', p.value < 0 && 'pulse-negative')}
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Projeção {p.label}</span>
              <div className={cn('w-7 h-7 rounded-lg flex items-center justify-center', p.value >= 0 ? 'bg-success/10' : 'bg-destructive/10')}>
                <TrendingUp className={cn('w-3.5 h-3.5', p.value >= 0 ? 'text-success' : 'text-destructive')} />
              </div>
            </div>
            <p className={cn('stat-value', p.value >= 0 ? 'text-success' : 'text-destructive')}>{formatCurrency(p.value)}</p>
            <p className={cn('text-[10px] mt-1', p.delta >= 0 ? 'text-success' : 'text-destructive')}>
              {p.delta >= 0 ? '▲' : '▼'} {formatCurrency(Math.abs(p.delta))} vs hoje
            </p>
          </motion.div>
        ))}
      </div>

      {/* Row 2: Fluxo semanal resumido */}
      <div className="grid grid-cols-3 gap-3">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className={cn('card-elevated p-4', stats.overdueCount > 0 && 'border-destructive/30')}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Saídas 7d</span>
            <ArrowDownCircle className={cn('w-4 h-4', stats.overdueCount > 0 ? 'text-destructive' : 'text-muted-foreground')} />
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="card-elevated p-4">
          <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-[10px] text-muted-foreground mt-1">
            {stats.inWeek >= stats.outWeek
              ? <span className="text-success">Superávit semanal</span>
              : <span className="text-destructive">Déficit semanal</span>}
          </p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card-elevated p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Saldo Líq. 7d</span>
            <TrendingUp className={cn('w-4 h-4', stats.inWeek - stats.outWeek >= 0 ? 'text-success' : 'text-destructive')} />
          </div>
          <p className={cn('stat-value', stats.inWeek - stats.outWeek >= 0 ? 'text-success' : 'text-destructive')}>
            {stats.inWeek - stats.outWeek >= 0 ? '+' : '−'}{formatCurrency(Math.abs(stats.inWeek - stats.outWeek))}
          </p>
